...
int main() {
    ...
    raylib::Rectangle boxA{
        {.x = 10, .y = static_cast<float>(::GetScreenHeight()) / 2.0F - 50.0F, .width = 200, .height = 100}};
    int boxASpeedX = 4;

    // Box B: Mouse moved box
    raylib::Rectangle boxB{
        {.x = static_cast<float>(::GetScreenWidth()) / 2.0F - 30.0F,
         .y = static_cast<float>(::GetScreenHeight()) / 2.0F - 30.0F,
         .width = 60,
         .height = 60}};
    ...
    while (!window.ShouldClose()) { // Detect window close button or ESC key
        ...
        if ((boxB.x + boxB.width) >= static_cast<float>(::GetScreenWidth()))
            boxB.x = static_cast<float>(::GetScreenWidth()) - boxB.width;
        else if (boxB.x <= 0)
            boxB.x = 0;

        if ((boxB.y + boxB.height) >= static_cast<float>(::GetScreenHeight()))
            boxB.y = static_cast<float>(::GetScreenHeight()) - boxB.height;
        ...
        if (collision) {
            ...
            // Draw collision area
            raylib::DrawText(
                ::TextFormat("Collision Area: %i", static_cast<int>(boxCollision.width * boxCollision.height)),
