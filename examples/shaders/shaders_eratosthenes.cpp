...
int main() {
    ...
    raylib::Shader shader(raylib::Shader::LoadShaderOptions{
        .fsFileName = TextFormat("resources/shaders/glsl%i/eratosthenes.fs", GLSL_VERSION)});
    ...
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        DrawTextureRec(
            target.GetTexture().c_raylib(),
            (Rectangle){0, 0, (float)target.GetTexture().GetWidth(), (float)-target.GetTexture().GetHeight()},
            (Vector2){0.0f, 0.0f},
            WHITE);
        shader.EndMode();
        EndDrawing();
