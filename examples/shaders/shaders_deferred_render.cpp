...
inline constexpr size_t MaxCubes = 30;
...
struct GBuffer {
    ...
    unsigned int albedoSpecTexture{0};
...
int main() {
    ...
    raylib::Model model(LoadModelFromMesh(GenMeshPlane(10.0f, 10.0f, 3, 3)));
    raylib::Model cube(LoadModelFromMesh(GenMeshCube(2.0f, 2.0f, 2.0f)));

    // Load geometry buffer (G-buffer) shader and deferred shader
    raylib::Shader gbufferShader(
        LoadShader("resources/shaders/glsl330/gbuffer.vs", "resources/shaders/glsl330/gbuffer.fs"));

    raylib::Shader deferredShader(
        LoadShader("resources/shaders/glsl330/deferred_shading.vs", "resources/shaders/glsl330/deferred_shading.fs"));
    ...
    rlSetUniformSampler(rlGetLocationUniform(deferredShader.GetId(), "gPosition"), 0);
    rlSetUniformSampler(rlGetLocationUniform(deferredShader.GetId(), "gNormal"), 1);
    rlSetUniformSampler(rlGetLocationUniform(deferredShader.GetId(), "gAlbedoSpec"), 2);

    rlDisableShader();

    // Assign out lighting shader to model
    model.SetMaterialShader(0, gbufferShader, raylib::ModelMaterialShaderOption::NoUnload);
    cube.SetMaterialShader(0, gbufferShader, raylib::ModelMaterialShaderOption::NoUnload);
    ...
    std::array<raylib::Vector3, MaxCubes> cubePositions;
    std::array<float, MaxCubes> cubeRotations;
    ...
    DeferredMode mode{DeferredMode::Shading};
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsKeyPressed(KEY_ONE)) mode = DeferredMode::Position;
        if (IsKeyPressed(KEY_TWO)) mode = DeferredMode::Normal;
        if (IsKeyPressed(KEY_THREE)) mode = DeferredMode::Albedo;
        if (IsKeyPressed(KEY_FOUR)) mode = DeferredMode::Shading;

        // Update light values (actually, only enable/disable them)
        for (const auto& light : lights) UpdateLightValues(deferredShader.c_raylib(), light);
        //----------------------------------------------------------------------------------
        ...
        rlEnableShader(gbufferShader.GetId());
        // When drawing a model here, make sure that the material's shaders
        // are set to the gbuffer shader!
        model.Draw(Vector3Zero(), 1.0f, WHITE);
        cube.Draw((Vector3){0.0, 1.0f, 0.0}, 1.0f, WHITE);
        ...
        switch (mode) {
            case DeferredMode::Shading:
            {
                ...
                /// @FIXME: rlBindFramebuffer not found
