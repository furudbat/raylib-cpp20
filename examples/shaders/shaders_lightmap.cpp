...
int main()
{
    ...
    // X                            // Y
    mesh.GetTexCoord2(0) = 0.0f;    mesh.GetTexCoord2(1) = 0.0f;
    mesh.GetTexCoord2(2) = 1.0f;    mesh.GetTexCoord2(3) = 0.0f;
    mesh.GetTexCoord2(4) = 0.0f;    mesh.GetTexCoord2(5) = 1.0f;
    mesh.GetTexCoord2(6) = 1.0f;    mesh.GetTexCoord2(7) = 1.0f;

    // Load a new texcoords2 attributes buffer
    mesh.SetVboId(SHADER_LOC_VERTEX_TEXCOORD02, rlLoadVertexBuffer(mesh.GetTexCoords2(), mesh.GetVertexCount()*2*sizeof(float), false));
    ...
    raylib::Shader shader (LoadShader(TextFormat("resources/shaders/glsl%i/lightmap.vs", GLSL_VERSION),
