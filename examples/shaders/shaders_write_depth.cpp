...
int main()
{
    ...
    while (!WindowShouldClose())        // Detect window close button or ESC key
    {
        ...
        DrawTextureRec(target.GetTextureC(), Rectangle{ 0, 0, (float)screenWidth, (float)-screenHeight }, Vector2{ 0, 0 }, WHITE);
        DrawFPS(10, 10);
        EndDrawing();
        //----------------------------------------------------------------------------------
    }
...
{
    raylib::RenderTexture2D target (rlLoadFramebuffer(width, height), raylib::RenderTextureOptions::UnloadFramebuffer); // Load an empty framebuffer
    ...
    if (target.GetId() > 0)
    {
        ...
        rlFramebufferAttach(target.GetId(), target.GetTextureC().id, RL_ATTACHMENT_COLOR_CHANNEL0, RL_ATTACHMENT_TEXTURE2D, 0);
        rlFramebufferAttach(target.GetId(), target.GetDepthC().id, RL_ATTACHMENT_DEPTH, RL_ATTACHMENT_TEXTURE2D, 0);

        // Check if fbo is complete with attachments (valid)
        if (rlFramebufferComplete(target.GetId())) TRACELOG(LOG_INFO, "FBO: [ID %i] Framebuffer object created successfully", target.id);
