...
int main() {
    ...
    parrots.Draw(
        ...
        raylib::Rectangle{
            {.x = 0,
             .y = 0,
             .width = static_cast<float>(cat.GetWidth()),
             .height = static_cast<float>(cat.GetHeight())}},
        raylib::Rectangle{
            {.x = 30,
             .y = 40,
             .width = static_cast<float>(cat.GetWidth()) * 1.5F,
             .height = static_cast<float>(cat.GetHeight()) * 1.5F}});
    parrots.Crop(raylib::Rectangle{
        {.x = 0,
         .y = 50,
         .width = static_cast<float>(parrots.GetWidth()),
    ...
    parrots.DrawText(
        font.c_raylib(),
        "PARROTS & CAT",
        raylib::Vector2{{.x = 300, .y = 230}},
        static_cast<float>(font.GetBaseSize()),
        -2);
    ...
    while (!window.ShouldClose()) { // Detect window close button or ESC key
        ...
        BeginDrawing();
        {
            ...
            texture.Draw(ScreenWidth / 2 - texture.GetWidth() / 2, ScreenHeight / 2 - texture.GetHeight() / 2 - 40);
            darkGray.DrawRectangleLines(
