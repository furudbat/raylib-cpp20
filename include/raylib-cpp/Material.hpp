...
enum class MaterialShaderOption : uint8_t {
    ...
    UnbindShaderWhenUnload =
        2, ///< Unbind (disconnect) shader from Material, to avoid UnloadMaterial() trying to unload it automatically (NO UnloadShader in UnloadingMaterial)
};
...
struct RayMaterials {
    ...
    operator std::span<::Material>() { return {data, size}; }
    operator std::span<const ::Material>() const { return {data, size}; }
...
public:
    ...
    explicit operator ::Material() const { return m_data; }
    [[nodiscard]] ::Material c_raylib() const& { return m_data; }
    ...
    void
    SetShader(Shader&& shader, MaterialShaderOption option = MaterialShaderOption::UnloadShaderWhenUnloadingMaterial) {
        m_data.shader = shader.c_raylib();
        m_trackShaderManagement = option;
        switch (m_trackShaderManagement) {
            case MaterialShaderOption::UnloadShaderWhenUnloadingMaterial:
                ...
                break;
            case MaterialShaderOption::UnbindShaderWhenUnload: break;
        }
    }
    ...
    Material& SetShaderValueFromLoc(
        size_t loc_index,
        std::variant<
            float,
            std::array<float, 2>, ///< vec2 (2 float)
    ...
    [[nodiscard]] const ::MaterialMap& GetMap(size_t index) const { return m_data.maps[index]; }
    [[nodiscard]] ::MaterialMap& GetMap(size_t index) { return m_data.maps[index]; }

    [[nodiscard]] const ::MaterialMap& GetMap(MaterialMapIndexT index) const {
        return m_data.maps[static_cast<size_t>(index)];
    }
    [[nodiscard]] ::MaterialMap& GetMap(MaterialMapIndexT index) { return m_data.maps[static_cast<size_t>(index)]; }
    ...
    [[nodiscard]] constexpr std::span<float> GetParams() { return std::span<float>{m_data.params, 4}; }
    ...
    void Unload() {
        switch (m_trackShaderManagement) {
            case MaterialShaderOption::UnloadShaderWhenUnloadingMaterial: break;
     ...
     */
    void DrawMesh(const ::Mesh& mesh, const ::Matrix& transform) const { ::DrawMesh(mesh, m_data, transform); }
    void DrawMesh(const Mesh& mesh, const ::Matrix& transform) const { ::DrawMesh(mesh.c_raylib(), m_data, transform); }
     ...
     */
    [[nodiscard]] bool IsReady() const { return ::IsMaterialReady(m_data); }
protected:
