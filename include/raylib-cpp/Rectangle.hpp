...
public:
    ...
    constexpr Rectangle(float _x, float _y, float _width, float _height)
        : ::Rectangle{_x, _y, _width, _height} {}
    [[deprecated("Use Rectangle(rect), use named-parameter struct")]]
    constexpr Rectangle(float _x, float _y, float _width)
        : ::Rectangle{_x, _y, _width, 0} {}
    [[deprecated("Use Rectangle(rect), use named-parameter struct")]]
    constexpr Rectangle(float _x, float _y)
        : ::Rectangle{_x, _y, 0, 0} {}
    [[deprecated("Use Rectangle(rect), use named-parameter struct")]]
    explicit constexpr Rectangle(float _x)
        : ::Rectangle{_x, 0, 0, 0} {}
    ...
    */
    ...
    [[nodiscard]] constexpr Vector4 ToVector4() { return Vector4{{.x = x, .y = y, .z = width, .w = height}}; }

    explicit constexpr operator raylib::Vector4() const { return Vector4{{.x = x, .y = y, .z = width, .w = height}}; }
     ...
     */
    [[nodiscard]] Rectangle GetCollision(::Rectangle rec2) const { return Rectangle{::GetCollisionRec(*this, rec2)}; }
    ...
    [[nodiscard]] constexpr Vector2 GetSize() const { return Vector2{{.x = width, .y = height}}; }
    ...
    constexpr Rectangle& SetSize(::Vector2 size) noexcept { return SetSize(size.x, size.y); }
    /*
    ...
    */
    ...
    [[nodiscard]] constexpr Vector2 GetPosition() const noexcept { return Vector2{{.x = x, .y = y}}; }
    ...
    constexpr Rectangle& SetPosition(::Vector2 position) noexcept { return SetPosition(position.x, position.y); }
protected:
